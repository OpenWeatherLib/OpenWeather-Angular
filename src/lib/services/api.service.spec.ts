...
describe("ApiService", () => {
    ...
    it("uvIndex should return expected json", (done: DoneFn) => {
        ...
        const req = httpMock.expectOne(`http://api.openweathermap.org/data/2.5/uvi?lat=${MockValues.city().geoLocation.lat.toFixed(2)}&lon=${MockValues.city().geoLocation.long.toFixed(2)}&APPID=${MockValues.apiKey()}`);
        expect(req.request.method).toEqual("GET");
        req.flush(expectedJson);
        httpMock.verify();
    });
